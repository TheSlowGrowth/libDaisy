...
void UI::ProcessEvent(const UiEventQueue::Event& e)
{
    switch(e.type)
    {
        ...
        case UiEventQueue::Event::EventType::buttonReleased:
            ForwardToButtonHandler(e.asButtonReleased.id, 0);
            break;
        case UiEventQueue::Event::EventType::encoderTurned:
            for(int32_t i = pages_.GetNumElements() - 1; i >= 0; i--)
            {
                ...
                if(pages_[i]->OnEncoderTurned(id, increments, stepsPerRev))
                    break;
            }
            break;
        case UiEventQueue::Event::EventType::encoderActivityChanged:
            for(int32_t i = pages_.GetNumElements() - 1; i >= 0; i--)
            {
                ...
                if(pages_[i]->OnEncoderActivityChanged(id, IsActive))
                    break;
            }
            break;
        case UiEventQueue::Event::EventType::potMoved:
            for(int32_t i = pages_.GetNumElements() - 1; i >= 0; i--)
            {
                ...
                if(pages_[i]->OnPotMoved(id, newPosition))
                    break;
            }
            break;
        case UiEventQueue::Event::EventType::potActivityChanged:
            for(int32_t i = pages_.GetNumElements() - 1; i >= 0; i--)
            {
                ...
                const bool IsActive
                    = activityType == UiEventQueue::Event::ActivityType::active;
                if(pages_[i]->OnPotActivityChanged(id, IsActive))
                    break;
            }
            break;
                                ...
                                const uint8_t  numberOfPresses)
{
    if(buttonID == specialControlIds_.okBttnId)
    {
        ...
        {
            if(pages_[i]->OnOkayButton(numberOfPresses))
                break;
        }
    }
    else if(buttonID == specialControlIds_.cancelBttnId)
    {
        for(int i = int(pages_.GetNumElements()) - 1; i >= 0; i--)
        {
            if(pages_[i]->OnCancelButton(numberOfPresses))
                break;
        }
    }
    else if(buttonID == specialControlIds_.funcBttnId)
    {
        for(int i = int(pages_.GetNumElements()) - 1; i >= 0; i--)
        {
            if(pages_[i]->OnFunctionButton(numberOfPresses))
                break;
        }
    }
    else if(buttonID == specialControlIds_.leftBttnId)
    {
        for(int i = int(pages_.GetNumElements()) - 1; i >= 0; i--)
        {
            if(pages_[i]->OnArrowButton(ArrowButtonType::left, numberOfPresses))
                break;
        }
    }
    ...
    else if(buttonID == specialControlIds_.upBttnId)
    {
        for(int i = int(pages_.GetNumElements()) - 1; i >= 0; i--)
        {
            if(pages_[i]->OnArrowButton(ArrowButtonType::up, numberOfPresses))
                break;
        }
    }
    else if(buttonID == specialControlIds_.downBttnId)
    {
        for(int i = int(pages_.GetNumElements()) - 1; i >= 0; i--)
        {
            if(pages_[i]->OnArrowButton(ArrowButtonType::down, numberOfPresses))
                break;
        }
    }
    else
    {
        for(int i = int(pages_.GetNumElements()) - 1; i >= 0; i--)
        {
            if(pages_[i]->OnButton(buttonID, numberOfPresses))
                break;
        }
    }
}

void UI::SetLookAndFeel(OneBitGraphicsLookAndFeel* lookAndFeel)
{
    if(lookAndFeel)
