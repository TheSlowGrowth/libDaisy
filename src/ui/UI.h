...
class UiPage
{
  public:
     ...
     */
    virtual bool OnOkayButton(uint8_t numberOfPresses)
    {
     ...
     *  @returns	false, if you want the event to be passed on to the page below.
     */
    virtual bool OnFunctionButton(uint8_t numberOfPresses)
    {
        (void)(numberOfPresses); // silence unused variable warnings
        return true;
    }
     ...
     *  @returns	false, if you want the event to be passed on to the page below.
     */
    virtual bool OnButton(uint16_t buttonID, uint8_t numberOfPresses)
    {
...
class UI
{
  public:
    ...
    /** Changes the LookAndFeel that should be used on this UI. 
     *  Pass nullptr to reset this to a default implementation.
    */
    void SetLookAndFeel(OneBitGraphicsLookAndFeel* lookAndFeel);

    /** Returns the lookAndFeel that's currently in use. */
    const OneBitGraphicsLookAndFeel& GetLookAndFeel() const
    {
        return *lookAndFeel_;
    }
  ...
  private:
    ...
    void ForwardToButtonHandler(uint16_t buttonID, uint8_t numberOfPresses);
...
#include "hid/disp/lookAndFeel.h"